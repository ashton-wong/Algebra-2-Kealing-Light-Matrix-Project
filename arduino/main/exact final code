...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(450, 6, NEO_GRB + NEO_KHZ800);  //44 Needs to be changed to 330 when running reg. code. Pin (3) matters too
void setup() {
  pixels.begin();
  pixels.show();
   ...
   for(int i=iStart; i>-1; i++){
      ...
      for (int j=0; j<clm_num; j++){
          ...
          for(int k=kStart; k>-2; k++){
                if ((j==clm_num-1) && (k == 10 + floor(length/rain_space))) {
                i=-2; //resets i
                k=-3;
                } else {
                    if (k == -1) {
                    ...
                    if (j%2 == 0) {
                        if (pixelPlace >= 0 && pixelPlace<= length-1) {
                      ...
                      }else if ((pixelPlace >= length) && (pixelPlace <= length + 2)) {
                          pixels.setPixelColor((j*length)+pixelPlace-3, pixels.Color(0,0,0));
                          if (pixelPlace <= length +1) {
                              pixels.setPixelColor((j*length)+pixelPlace-2, pixels.Color(topRed,topGreen,topBlue));
                              if (pixelPlace <= length) {
                                  pixels.setPixelColor((j*length)+pixelPlace-1, pixels.Color(midRed,midGreen,midBlue));
                              }
                          }
                    ...
                    if (j%2 == 1) {//could be else, kept for readabilityhttps://circuits.io/help
                        if (pixelPlace >= 0 && pixelPlace <= length-1) {
