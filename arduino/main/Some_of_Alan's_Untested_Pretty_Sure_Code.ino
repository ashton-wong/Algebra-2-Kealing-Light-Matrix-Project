...
void loop() {
  ...
  int camera [10] = {11,11,11,11,11,11,11,11,11,11}; // each of these values is the y value of the edge divided to split into 45 parts, x values in (divided by 10)
  ...
  for(int i=iStart; i>-2; i++){
    //The next bit of code is fake to see if this works on circuits.io
    int serial [10] = {0,1,4,5};//for testing, needs to be deleted
    for (int l=0; l<clm_num; l++){
      camera [l] = length - serial [l];
    }
    //while (!Serial) {//Wait for connection
    //}
    //for (int l=0; l,clm_num; l++){
    //int inChar = length-Serial.read(l);
    //camera [l] = serial [l];
    //}
        ...
        if (i == 89) {
          ...
          if (j%2 == 0) {
            if ((pixelPlace >= camera [j]) && (pixelPlace <= camera [j] + 2)) {//length is an arbitrary location where the rain stops. Cough Cough Hint Hint
              ...
              k=-3;
            } else if ((pixelPlace >= camera [j])&&(pixelPlace<length)) {
